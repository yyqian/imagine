 ...
 * 7. Both abstract and concrete classes can be entities. Entities may extend non-entity classes as well as entity classes,
 * and non-entity classes may extend entity classes.
 * <p>
 * Difference in Hibernate requirements:
 * 1. The entity class must have a no-argument constructor, which may be public, protected or package visibility.
 * 2. The entity class need not be a top-level class.
 * 3. Technically Hibernate can persist final classes or classes with final persistent state accessor (getter/setter) methods.
 * <p>
 ...
 * 3. Declare getters and setters for persistent attributes. Attributes (whether fields or getters/setters) need not be declared public.
 ...
 * @Type: If you do not specify a type, Hibernate will use reflection upon the named property and guess the correct Hibernate type.
 * <p>
 ...
 * 3. if the type of the property is Serializable, it is mapped as @Basic in a column holding the object in its serialized version
 * 4. if the type of the property is java.sql.Clob or java.sql.Blob, it is mapped as @Lob with the appropriate LobType
 * <p>
 ...
 * - inverse side must refer to its owning side by using mappedBy element of @OneToOne, @OneToMany or @ManyToMany.
 ...
 * <p>
 * Unidirectional relationship: only one entity has a relationship field/property (which is annotated)
 * <p>
 * Query: navigation depends on Bidirectional or Unidirectional. If unidirectional, only the side has a relationship field/property can navigate across relationships.
 * <p>
 * Cascade Operations:
 * <p>
 ...
 * }
 * <p>
 * If one comment is deleted from the comment list in a post (the deletion happens in the post entity),
 ...
 * <p>
 * Abstract entity:
 * - Abstract entity cannot be instantiated, but can be queried. If an abstract entity is the target of a query,
 * - the query operates on all the concrete subclasses of the abstract entity.
 * - Abstract entity do have a corresponding table?
 * @MappedSuperclass: - Mapped superclasses cannot be queried and cannot be used in EntityManager or Query operations.
 ...
 * - TABLE_PER_CLASS (DON'T USE IT), a table per concrete entity class, may NOT be supported by some JPA providers.
 */
...
public abstract class Base implements Serializable {
  ...
  /**
   * In SQL Database, name "created_at" is preferred, instead of "createdAt", so we need to define it manually.
   * In plain Java APIs, the temporal precision of time is not defined. When dealing with temporal data you might
   * want to describe the expected precision in database. Temporal data can have DATE, TIME, or TIMESTAMP precision
   ...
   * java.sql.Date: year, month, day (hour, minute, second and millisecond are ignored, isn't tied to timezones)
   * java.sql.Time: hrs, min, sec (only contains information about hour, minutes, seconds and milliseconds.)
   * java.sql.Timestamp: year, month, day, hrs, min, sec, nano sec (exact date to the nanosecond, note that util.Date only supports milliseconds)
   */
